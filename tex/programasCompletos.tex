...
\end{lstlisting}
\inputencoding{utf8}
\section{√Årboles de expresiones}
...
def obtenerNumero(listaLexemas):
  if obtenerLexema(listaLexemas, "("):
    # obtiene la subexpresiÛn
    x = obtenerSuma(listaLexemas) 
    # elimina los parÈntesis
...
def animal():
  ...
  while True:
    ...
    pregunta = "øEs un" + conjetura + "? "
    if si(pregunta):
      print("°Soy el mejor!")
      continue
    # obtener mas informacion
    pregunta = "øCual es el nombre el animal? "
    animal = raw_input(pregunta)
    pregunta = "øQue pregunta permitiria distinguir 
		un %s de un %s? "
    q = raw_input(pregunta % (animal,conjetura))
    # agrega un nuevo nodo arbol
    arbol.asignarcarga(q)
    pregunta = "øSi el animal fuera %s 
		la respuesta serÌa? "
